...
Net effect: federal property-tax deduction is \$0 (state income tax alone exhausts the \$10K floor at top-bracket income). At lower federal-marginal-bracket selections the SALT cap may not fully phase down, but the model still treats federal property-tax deduction as \$0 — a documented simplification (see [Simplifications](#simplifications) #16).
...
Federal property-tax deduction is \$0 (the \$10K SALT floor is consumed by state income tax at top-bracket federal income). State property-tax deduction folds into the state itemized total above; equivalently:
...
16. **Federal SALT cap modeled at the \$10K phase-down floor across all federal-marginal-bracket selections.** The 2026 headline cap of \$40,400 phases down at 30% of MAGI excess above \$505K MAGI to a \$10K floor; only at top-bracket federal income (37% bracket → MAGI ≥ \$731K MFJ) does the cap fully bottom out. At the 22% / 24% / 32% / 35% federal-marginal selections, real MAGI may not exceed the phase-down threshold and the effective SALT cap would sit between \$10K and \$40,400. The model uses the \$10K floor uniformly. Net effect: federal property-tax deduction is treated as \$0 (state income tax alone exhausts the floor) regardless of bracket selection. At lower brackets this slightly understates Buy's tax savings; the magnitude depends on actual MAGI, which the model doesn't separately track. **No-income-tax states (AK, FL, NV, NH, SD, TN, TX, WA, WY):** the "state income tax exhausts the \$10K floor" assumption breaks down here — with no state income tax to absorb the floor, the federal SALT cap should allow up to \$10K of property-tax deduction (or \$10K minus other SALT items if any). The model still treats federal property-tax deduction as \$0 in these states, which underestimates Buy's federal tax savings by at most \$10K × federal marginal per year (≈ \$2.2K-\$3.7K).
